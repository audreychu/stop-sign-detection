...
yespathlist = []
for p in glob.glob('YES - Present2/*.jpg'):
    yespathlist.append(p)
    
nopathlist = []
for p in glob.glob('NO - Absent2/*.jpg'):
    nopathlist.append(p)
...
convnet = regression(convnet, loss='categorical_crossentropy', learning_rate=0.001, name = 'regression')
...
'''if os.path.exists('stopsign.meta'):
    ...
    print('model loaded')
'''
model.fit(xtrain,ytrain, n_epoch = 100, snapshot_epoch = True, run_id = 'stopsign7', validation_set=(xvalid,yvalid),snapshot_step = 50, show_metric = True)

    
model.save('C://Users/Jeremy/Desktop/School/160/stopsign7.model')